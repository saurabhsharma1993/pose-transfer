...
class Block(nn.Module):
    ...
    def build_net(self, input_nc, output_nc, down=True, bn=True, dropout=False, leaky=True):
        ...
        if down:
            model.append(nn.Conv2d(input_nc, output_nc, kernel_size=4, stride=2, padding=1, bias=True))
        else:
            model.append(nn.ConvTranspose2d(input_nc, output_nc, kernel_size=4, stride=2, bias=True))
            model.append(Cropping2D(1))
        if bn:
            model.append(nn.InstanceNorm2d(output_nc))
        if dropout:
