...
if __name__ == "__main__":
    ...
    if(opt.checkMode !=0):
        pairs_df_train = pairs_df_train.sample(n=min(4000, pairs_df_train.shape[0]), replace=False,                                                                            random_state=0)
        print ('Number of pairs: %s' % len(pairs_df_train))
        pairs_df_train.to_csv(opt.pairs_file_train_check, index=False)
    else:
        print('Number of pairs: %s' % len(pairs_df_train))
    ...
    if(opt.checkMode !=0):
        pairs_df_test = pairs_df_test.sample(n=min(4000, pairs_df_test.shape[0]), replace=False,
                                             random_state=0)
        print('Number of pairs: %s' % len(pairs_df_test))
        pairs_df_test.to_csv(opt.pairs_file_test_check, index=False)
    else:
